...
app.get('/', (request, response, next) => {
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		registerdb.collection("springregister").find({}).toArray((err, itemsArray) => {
			if (err)
...
app.get('/springregister/:student_id', (request, response, next) => {
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		//read from registerdb students collection
		registerdb.collection("springregister").find(query).toArray((err, itemsArray) => {
			if (err)
				console.log(err.message);
...
app.get('/course_id', (request, response, next) => {
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		//read from registerdb students collection
		registerdb.collection("springregister").find({course_id:"CIA 3303"}).toArray((err, itemsArray) => {
			if (err)
				console.log(err.message);
...
app.post('/springregister', (request, response, next) => {
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		registerdb.collection("springregister").insertOne(newstudent, (err, result) => {
			if (err) {
				console.log(err.message);
			}
			...
			if (result.insertedCount == 1) {
				...
				response.redirect("/static/springregister.html");
			}
			else
...
app.put('/springregister', (request, response, next) => {
	console.log("in PUT");
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		registerdb.collection("springregister").updateOne(updateFilter, updateValues, (err, res) => {
			if (err) {
				console.log(err.message);
			}
...
app.delete('/springregister', (request, response, next) => {
	const student_id = request.param('student_id');
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		//insert from registerdb students collection
		registerdb.collection("springregister").deleteOne(deleteFilter, (err, res) => {
			if (err) {
				console.log(err.message);
			}
