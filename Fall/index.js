...
app.get('/', (request, response, next) => {
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		registerdb.collection("fallregister").find({}).toArray((err, itemsArray) => {
			if (err)
...
app.get('/fallregister/:student_id', (request, response, next) => {
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		//read from registerdb students collection
		registerdb.collection("fallregister").find(query).toArray((err, itemsArray) => {
			if (err)
				console.log(err.message);
...
app.get('/course_id', (request, response, next) => {
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		//read from registerdb students collection
		registerdb.collection("fallregister").find({course_id:"CIA 3303"}).toArray((err, itemsArray) => {
			if (err)
				console.log(err.message);
...
app.post('/fallregister', (request, response, next) => {
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		registerdb.collection("fallregister").insertOne(newstudent, (err, result) => {
			if (err) {
				console.log(err.message);
			}
			...
			if (result.insertedCount == 1) {
				...
				response.redirect("/static/fallregister.html");
			}
			else
...
app.put('/fallregister', (request, response, next) => {
	console.log("in PUT");
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		registerdb.collection("fallregister").updateOne(updateFilter, updateValues, (err, res) => {
			if (err) {
				console.log(err.message);
			}
...
app.delete('/fallregister', (request, response, next) => {
	const student_id = request.param('student_id');
	...
	mongoClient.connect(mongoServerURL, (err, db) => {
		...
		//insert from registerdb students collection
		registerdb.collection("fallregister").deleteOne(deleteFilter, (err, res) => {
			if (err) {
				console.log(err.message);
			}
